...
export default function AppointmentLayouts() {
  ...
  const validation = useMemo(() => {
    ...
    layout.modal_layout.forEach((el) => {
      if (el.element === "section" && !el.title?.trim()) errs.modalEls[el.id] = true;
  ...
  const handleEdit = (layoutItem) => {
    ...
    const layoutData = layoutItem.layout_data || {};
    ...
    const loadedLayout = {
      ...
      modal_layout: layoutData.modal_layout || []
    };
    ...
    setOriginalState({
      ...
      ref: layoutItem.ref,
      isNew: false,
      layoutJson: JSON.stringify(cleanLayoutData(loadedLayout))
    });
    setViewMode("editor");
  };
  ...
  const handleCreateNew = () => {
    ...
    setOriginalState({
      ...
      ref: "",
      isNew: true,
      layoutJson: JSON.stringify(cleanLayoutData(freshLayout))
    });
    setViewMode("editor");
  };
  ...
  const handleJsonChange = (parsed) => {
    setLayout({
      ...
      modal_layout: (parsed.modal_layout || []).map(m => ({ ...m, id: m.id || uid() }))
    });
  };
