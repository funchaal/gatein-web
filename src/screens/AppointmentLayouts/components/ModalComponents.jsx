...
export function ModalSectionElement({ title }) {
  const styles = {
...
export function ModalFieldElement({ label, field, data }) {
  const styles = {
    container: { paddingTop: 8, paddingBottom: 8, display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
...
export function ModalAlertElement({ color, icon, title, field, data }) {
  const styles = {
    container: { borderRadius: 12, borderWidth: 1, borderStyle: 'solid', padding: 16, marginTop: 30 },
    content: { display: 'flex', flexDirection: 'row', alignItems: 'flex-start' },
    iconContainer: { marginRight: 12, marginTop: 2, display: 'flex', flexShrink: 0 },
  ...
  if (!value) {
    return (
      <div style={{ ...styles.container, backgroundColor: colorScheme.bg, borderColor: colorScheme.border, opacity: 0.7 }}>
        <div style={styles.content}>
          {icon && (
            <div style={styles.iconContainer}>
               <IconSvg name={icon || "information"} size={24} color={colorScheme.text} />
            </div>
          )}
  ...
  return (
    <div style={{ ...styles.container, backgroundColor: colorScheme.bg, borderColor: colorScheme.border }}>
      <div style={styles.content}>
        {icon && (
          <div style={styles.iconContainer}>
             <IconSvg name={icon || "information"} size={24} color={colorScheme.text} />
...
export function ModalQRCodeElement({ title, caption, field, data }) {
  const styles = {
    ...
    title: { fontSize: 15, fontWeight: 700, color: THEME.slate900, marginTop: 16, marginBottom: 16 },
